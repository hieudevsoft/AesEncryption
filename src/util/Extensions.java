package util;

import javax.accessibility.AccessibleBundle;
import javax.management.QueryEval;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.stream.Collectors;

public class Extensions {
    public static void showWord(int word) {
        for (int i = 1; i <= 8; i++) {
            int hex = (word >> 32 - i * 4) & 0xF;
            System.out.printf("%x", hex);
        }
        System.out.println();
    }

    public static int RotWord(int word) {
        int byte1 = (word >> 24) & 0xFF;
        int byte234 = word & 0xFFFFFF;
        return (byte234 << 8) | byte1;
    }

    public static int SubWord(int word) {
        int S_Box[] = {
                
        };
        int kq = 0;
        for (int i = 1; i <= 4; i++) {
            int bytei = (word >> 32 - i * 8) & 0xFF;
            int subB = S_Box[bytei];
            kq = (kq << 8) | subB;
        }
        return kq;
    }

    public static int XorRSub(int w, int j) {
        int Rc[] = {
                0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a,
                0x2f, 0x5e, 0xbc, 0x63, 0xc6, 0x97, 0x35, 0x6a, 0xd4, 0xb3, 0x7d, 0xfa, 0xef, 0xc5, 0x91, 0x39
        };
        int byte1 = (w >> 24) & 0xFF;
        int resultXor = byte1 ^ Rc[j];
        int byte234 = w & 0xFFFFFF;
        int kq = (resultXor << 24) | byte234;
        return kq;
    }

    public static int G(int w, int j) {
        int rotW = RotWord(w);
        int subW = SubWord(rotW);
        int kq = XorRSub(subW, j);
        return kq;
    }

    public static int[] KeyExpansion(List<Integer> keys, int nK, int nR) {
        int[] w = new int[nK * (nR + 1)];
        for (int i = 0; i < nK; i++)
            w[i] = keys.get(i);
        for (int i = nK; i < nK * (nR + 1); i++) {
            if (i % nK == 0) w[i] = G(w[i - 1], i / 4) ^ w[i - 4];
            else w[i] = w[i - 1] ^ w[i - 4];
        }
        return w;
    }

    public static int[] AddRoundKey(List<Integer> states, List<Integer> keys) {
        int[] kq = new int[states.size()];
        kq[0] = keys.get(0) ^ states.get(0);
        kq[1] = keys.get(1) ^ states.get(1);
        kq[2] = keys.get(2) ^ states.get(2);
        kq[3] = keys.get(3) ^ states.get(3);
        return kq;
    }

    public static int[] SubBytes(List<Integer> states) {
        int[] kq = new int[states.size()];
        for (int i = 0; i < states.size(); i++)
            kq[i] = SubWord(states.get(i));
        return kq;
    }

    public static int[] ShiftRows(List<Integer> states) {
        int[] kq = new int[states.size()];
        for (int i = 0; i < states.size(); i++) {
            int byte1 = states.get(i) & 0xFF000000;
            int byte2 = states.get((i + 1) % 4) & 0xFF0000;
            int byte3 = states.get((i + 2) % 4) & 0xFF00;
            int byte4 = states.get((i + 3) % 4) & 0xFF;
            kq[i] = byte1 | byte2 | byte3 | byte4;
        }
        return kq;
    }

    public static int Mul2(int w) {
        int kq = w << 1;
        if (kq > 256) kq = kq ^ 0x11b;
        kq = kq & 0xFF;
        return kq;
    }

    public static int Mul3(int w) {
        int kq = w ^ Mul2(w);
        kq = kq & 0xFF;
        return kq;
    }

    private static int MulColumn(int w) {
        int kq;
        int byte1 = (w >> 24) & 0xFF;
        int byte2 = (w >> 16) & 0xFF;
        int byte3 = (w >> 8) & 0xFF;
        int byte4 = w & 0xFF;
        int kq1 = Mul2(byte1) ^ Mul3(byte2) ^ byte3 ^ byte4;
        int kq2 = byte1 ^ Mul2(byte2) ^ Mul3(byte3) ^ byte4;
        int kq3 = byte1 ^ byte2 ^ Mul2(byte3) ^ Mul3(byte4);
        int kq4 = Mul3(byte1) ^ byte2 ^ byte3 ^ Mul2(byte4);
        kq = (kq1 << 24) | (kq2 << 16) | (kq3 << 8) | kq4;
        return kq;
    }

    public static int[] MixColumns(List<Integer> states) {
        int[] kq = new int[states.size()];
        for (int i = 0; i < kq.length; i++)
            kq[i] = MulColumn(states.get(i));
        return kq;
    }

    public static void showMatrix(int[] states) {
        for (int i = 0; i < states.length; i++)
            Extensions.showWord(states[i]);
    }

    public static int[] EncryptionAes(List<Integer> states, List<Integer> keys, int nK, int nR) {
        int[] w = KeyExpansion(keys, nK, nR);
        int[] state = AddRoundKey(states, Arrays.stream(w).boxed().collect(Collectors.toList()));
        for (int j = 1; j <= nR; j++) {
            state = SubBytes(Arrays.stream(state).boxed().collect(Collectors.toList()));
            state = ShiftRows(Arrays.stream(state).boxed().collect(Collectors.toList()));
            if (j != nR) state = MixColumns(Arrays.stream(state).boxed().collect(Collectors.toList()));
            state = AddRoundKey(Arrays.stream(state).boxed().collect(Collectors.toList()), Arrays.stream(w).boxed().collect(Collectors.toList()).subList(nK * j, w.length));
        }
        return state;
    }

    /*Decode*/
    public static int[] InvShiftRow(List<Integer> states) {
        int[] kq = new int[states.size()];
        for (int i = 0; i < states.size(); i++) {
            int byte1 = states.get(i) & 0xFF000000;
            int byte2 = states.get((i + 3) % 4) & 0xFF0000;
            int byte3 = states.get((i + 2) % 4) & 0xFF00;
            int byte4 = states.get((i + 1) % 4) & 0xFF;
            kq[i] = byte1 | byte2 | byte3 | byte4;
        }
        return kq;
    }

    public static int InvSubWord(int w) {
        int InvS[] = {
                
        };
        int kq = 0;
        for (int i = 1; i <= 4; i++) {
            int bytei = (w >> 32 - i * 8) & 0xFF;
            int subB = InvS[bytei];
            kq = (kq << 8) | subB;
        }
        return kq;
    }

    public static int[] InvSubBytes(List<Integer> states) {
        int[] kq = new int[states.size()];
        for (int i = 0; i < states.size(); i++)
            kq[i] = InvSubWord(states.get(i));
        return kq;
    }

    public static int MulB(int w) {
        int kq = (w << 3) ^ (w << 1) ^ w;
        if (kq > (256 << 2)) kq = kq ^ (0x11b << 2);
        if (kq > (256 << 1)) kq = kq ^ (0x11b << 1);
        if (kq > (256)) kq = kq ^ 0x11b;
        kq = kq & 0xFF;
        return kq;
    }

    public static int MulD(int w) {
        int kq = (w << 3) ^ (w << 2) ^ w;
        if (kq > (256 << 2)) kq = kq ^ (0x11b << 2);
        if (kq > (256 << 1)) kq = kq ^ (0x11b << 1);
        if (kq > (256)) kq = kq ^ 0x11b;
        kq = kq & 0xFF;
        return kq;
    }

    public static int Mul9(int w) {
        int kq = (w << 3) ^ w;
        if (kq > (256 << 2)) kq = kq ^ (0x11b << 2);
        if (kq > (256 << 1)) kq = kq ^ (0x11b << 1);
        if (kq > (256)) kq = kq ^ 0x11b;
        kq = kq & 0xFF;
        return kq;
    }

    public static int MulE(int w) {
        int kq = (w << 3) ^ (w << 2) ^ (w << 1);
        if (kq > (256 << 2)) kq = kq ^ (0x11b << 2);
        if (kq > (256 << 1)) kq = kq ^ (0x11b << 1);
        if (kq > (256)) kq = kq ^ 0x11b;
        kq = kq & 0xFF;
        return kq;
    }

    private static int InvMulColumn(int w) {
        int kq;
        int byte1 = (w >> 24) & 0xFF;
        int byte2 = (w >> 16) & 0xFF;
        int byte3 = (w >> 8) & 0xFF;
        int byte4 = w & 0xFF;
        int kq1 = MulE(byte1) ^ MulB(byte2) ^ MulD(byte3) ^ Mul9(byte4);
        int kq2 = Mul9(byte1) ^ MulE(byte2) ^ MulB(byte3) ^ MulD(byte4);
        int kq3 = MulD(byte1) ^ Mul9(byte2) ^ MulE(byte3) ^ MulB(byte4);
        int kq4 = MulB(byte1) ^ MulD(byte2) ^ Mul9(byte3) ^ MulE(byte4);
        kq = (kq1 << 24) | (kq2 << 16) | (kq3 << 8) | kq4;
        return kq;
    }

    public static int[] InvMixColumns(List<Integer> states) {
        int[] kq = new int[states.size()];
        for (int i = 0; i < kq.length; i++)
            kq[i] = InvMulColumn(states.get(i));
        return kq;
    }

    public static int[] InvAes(List<Integer> states, List<Integer> keys, int nK, int nR) {
        int[] w = KeyExpansion(keys, nK, nR);
        int[] state = AddRoundKey(states, Arrays.stream(w).boxed().collect(Collectors.toList()).subList(nK * nR, w.length));
        for (int j = 1; j <= nR; j++) {
            state = InvShiftRow(Arrays.stream(state).boxed().collect(Collectors.toList()));
            state = InvSubBytes(Arrays.stream(state).boxed().collect(Collectors.toList()));
            state = AddRoundKey(Arrays.stream(state).boxed().collect(Collectors.toList()), Arrays.stream(w).boxed().collect(Collectors.toList()).subList(nK * (nR - j), w.length));
            if(j!=nR) state = InvMixColumns(Arrays.stream(state).boxed().collect(Collectors.toList()));
        }
        return state;
    }
}
